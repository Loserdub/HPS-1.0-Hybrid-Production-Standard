...
- **Manifest**: The canonical UTF-8 JSON document containing the declared axis states, derived tier code, creator identity parameters, audio essence digest, revision history, and RFC 8032 Ed25519 digital signature. Manifests are canonicalized using RFC 8785 JSON Canonicalization Scheme (JCS) prior to hashing.
...
- **Signature & Integrity Check (3-Point)**: The automated verification validating that (1) an audio file's PCM essence digest matches the manifest content hash, (2) the Ed25519 digital signature is cryptographically valid and untampered post-signing, and (3) the declared production tier matches the rigid deterministic rules of the 243-state classifier.
...
The schema incorporates native industry release identifiers, extraction confidence indicators, multi-party co-attestation envelopes, and an immutable Merkle tree revision history:
...
```json
{
  ...
  "signature": {
    ...
    "value": "9a2f1c...4b8e",
    "signer_role": "creator",
    "signed_at": "2026-08-05T12:00:00Z"
  },
  "signatures": [
    {
      "algorithm": "ed25519",
    ...
    },
    {
      ...
      "signed_at": "2026-08-05T12:05:00Z"
    }
  ]
}
```

### 5.2 Merkle Tree Revision History & Multi-Signer Co-Attestation Specification
To support complex music supply chain workflows (e.g., label delivery, publisher verification, co-writer split agreements), HPS-1.0 incorporates a dual cryptographic integrity model:

1. **SHA-256 Merkle Revision History (`revision_history[]`)**:
   Every state alteration or co-signature appends an entry to the `revision_history` array. The pure manifest state (excluding mutable signature envelopes) is hashed to compute `revision_hash`, linked to `parent_hash`, and compiled into a binary SHA-256 Merkle root (`merkle_root`). Downstream platforms can verify that no intermediate metadata was forged or silently altered after initial attestation.

2. **Multi-Signer Co-Attestation (`signatures[]`)**:
   While the primary creator signs with `signer_role: "creator"`, subsequent parties (such as record labels, distributors, or publishers) append cryptographically valid Ed25519 signatures to `signatures[]`. Each signature binds directly to the canonical RFC 8785 representation of the manifest and its immutable `content_hash`, allowing enterprise verification without altering the underlying audio essence hash.
...
- **Audio Essence Hashing**: The engine extracts the uncompressed PCM sample data of the final master render, computing a SHA-256 digest (`content_hash.value`). Modifying ID3 tags, BWF headers, or container metadata never invalidates the essence hash.
...
HPS-1.0 provides two complementary transmission mechanisms for attestation manifests: native container metadata embedding (active in current reference implementations) and an open theoretical reference specification for time-domain spread-spectrum acoustic watermarking:
...
- **Theoretical Acoustic Watermarking Specification**:
  > [!NOTE]
  > **Implementation Status Note**: In current production deployments, attestation manifests travel via native container chunks (`hps1`), ID3 tags, and DDEX ERN 4.3 XML sidecars. The acoustic watermarking architecture specified below represents a theoretical reference design designed for future hardware, DSP plugin, and codec-level standardization to survive lossy transcoding (e.g. AAC/MP3) and analog broadcast channels.
...
- **Merkle Tree Revision History**: A cryptographic data structure wherein sequential manifest state hashes (`revision_hash`) are organized into a tree whose apex (`merkle_root`) certifies that no intermediate metadata fields were manipulated post-signing.
...
- **Parse Confidence Tier**: A metadata attribute (`high`, `medium`, `partial`, `none`) indicating the depth and structural fidelity of automated session file extraction (e.g., native AST deserialization vs. binary string scraping vs. heuristic fallback).
...
- **Signature & Integrity Check (3-Point)**: An automated protocol validating that: (1) an audio file's raw PCM essence digest matches the manifest hash, (2) the Ed25519 digital signature is cryptographically valid, and (3) the declared production tier matches deterministic 243-state classifier rules.
